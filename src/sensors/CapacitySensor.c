...
static void capacity_measurement_task(void *pvParameters) {
    CapacitySensor_t *sensor = (CapacitySensor_t *)pvParameters;
    ...
    while (1) {
        ...
        if (xTaskNotifyWait(0x00, ULONG_MAX, &tof_ticks, pdMS_TO_TICKS(1000)) == pdTRUE) {
            ...
            float capacity_value = (float)pulse_width_us / 58;
...
void CapacitySensor_Start(CapacitySensor_t *sensor) {
    xTaskCreate(capacity_measurement_task, "capacity_task", 4096, sensor, 5, NULL);
}
